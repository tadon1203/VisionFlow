...
#ifdef _WIN32
class DmlImageProcessorInterop::Impl {
  public:
    ...
    initializeOrUpdate(ID3D11Texture2D* sourceTexture) {
        ...
        commit = true;
        rollback.dismiss();
        VF_DEBUG("DmlImageProcessorInterop initialized for {}x{} format={}", sourceDesc.Width,
  ...
  private:
    ...
    [[nodiscard]] DmlInteropUpdateResult buildUpdateResult(bool reinitializedState,
                                                           bool sourceChangedState) const {
        return DmlInteropUpdateResult{
    ...
    initializeD3d11State(const winrt::com_ptr<ID3D11Device>& incomingDevice,
        ...
        const auto queryContext4Result = dx_utils::toError(
    ...
    std::expected<void, std::error_code> initializeD3d12AndDmlState() {
        ...
        const auto createD3d12DeviceResult = dx_utils::toError(
        ...
        const auto createQueueResult = dx_utils::toError(
    ...
    std::expected<void, std::error_code> initializeSharedInputState() {
        ...
        const auto createSharedTextureResult = dx_utils::toError(
        ...
        const auto querySharedResourceResult = dx_utils::toError(
        ...
        const auto createSharedTextureHandleResult = dx_utils::toError(
        ...
        const auto openSharedTextureResult = dx_utils::toError(
    ...
    std::expected<void, std::error_code> initializeInteropFenceState() {
        const auto createInteropFenceResult = dx_utils::toError(
            dx_utils::checkD3d(d3d12Device->CreateFence(0, D3D12_FENCE_FLAG_SHARED,
        ...
        const auto createInteropFenceHandleResult = dx_utils::toError(
        ...
        const auto openInteropFenceResult = dx_utils::toError(
            dx_utils::checkD3d(d3d11Device5->OpenSharedFence(interopFenceHandle.get(),
...
DmlImageProcessorInterop::DmlImageProcessorInterop() : impl(std::make_unique<Impl>()) {}
DmlImageProcessorInterop::~DmlImageProcessorInterop() { impl->reset(); }

std::expected<DmlInteropUpdateResult, std::error_code>
DmlImageProcessorInterop::initializeOrUpdate(ID3D11Texture2D* sourceTexture) {
    return impl->initializeOrUpdate(sourceTexture);
}

std::expected<std::uint64_t, std::error_code>
DmlImageProcessorInterop::copyAndSignal(ID3D11Texture2D* frameTexture,
                                        std::uint64_t requestedFenceValue) {
...
ID3D12Device* DmlImageProcessorInterop::getD3d12Device() const { return impl->getD3d12Device(); }
ID3D12CommandQueue* DmlImageProcessorInterop::getD3d12Queue() const {
    return impl->getD3d12Queue();
}
IDMLDevice* DmlImageProcessorInterop::getDmlDevice() const { return impl->getDmlDevice(); }
ID3D12Resource* DmlImageProcessorInterop::getSharedInputTextureD3d12() const {
    return impl->getSharedInputTextureD3d12();
}
const D3D11_TEXTURE2D_DESC& DmlImageProcessorInterop::getSourceDesc() const {
    return impl->getSourceDescRef();
}

void DmlImageProcessorInterop::reset() { impl->reset(); }
...
DmlImageProcessorInterop::DmlImageProcessorInterop() : impl(std::make_unique<Impl>()) {}
DmlImageProcessorInterop::~DmlImageProcessorInterop() = default;
